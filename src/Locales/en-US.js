	...
	commands: {
		antiinvite: {
			...
			secondField: {
				...
				value: 'To configure the anti-invite module is very simple, just insert one of these options after the command: n> `activate` › Activate the modulen> `deactivate` › Deactivate the modulen**But like me do i set up a channel where invites will be ignored? Just use one of the whitelist options below:**n> `whitelist add` › Add a channel to the list of channels that the bot will ignore invitesn> `whitelist remove` › Remove a channel from the list of channels the bot ignores'
			},
			enabled: 'Anti-invitation module successfully activated!',
			disabled: 'Anti-invitation module successfully deactivated!',
			errorWhitleList: 'You need to put what you want to do inside the whitelist subcommandn> `whitelist add` › Add a channel to the blocked channel listn> `whitelist remove › Remove a channel from the blocked channel list',
		...
		welcome: {
			errorPerm: 'You need `Manage Server` permission to run this command!',
			argsError: 'You didn\'t use the command correctly use `<welcome on/off [channel] [incoming message]` if you want to know some placeholders check out this short list: n `{guild}` - Displays the name of the server. n `{member}` - Mention the user who entered n `{membertag}` - Display the name and tag of the user.n `{count}` - Display the number of members of the server.',
